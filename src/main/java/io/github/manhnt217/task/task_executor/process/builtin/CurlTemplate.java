...
public class CurlTemplate extends Template<CurlTemplate.Input, CurlTemplate.Output> {
	...
	private Output doRequest(String requestURL, String method, Map<String, String> headers, Map<String, Object> queryParams, Object payload) {
			...
			if (res.isSuccess()) {
				...
				return new Output(res.getStatus(),
						"Got an exception while making the request to portainer" +
								...
								". Code = " + res.getStatus() +
								". Message = " + res.getBody() +
								". Stacktrace = " + res.getParsingError().map(ExceptionUtils::getStackTrace).orElse("")
						);
			}
		} catch (JsonProcessingException e) {
			throw new RuntimeException("Got an exception while preparing data for request. URL = " + requestURL, e);
		}
	}
